...
sed -i.bak '/"eslint-config-strapi": "\^4\.15\.0"/d' backend/package.json
sed -i.bak '/"eslint-config-strapi": "\^4\.20\.0"/d' backend/package.json
sed -i.bak '/"eslint-plugin-strapi": "\^4\.15\.0"/d' backend/package.json
...
echo "- 移除不存在的 ESLint 配置包"
echo "- 使用标准的 ESLint 配置"
echo ""
echo "🔄 现在可以重新安装依赖："
